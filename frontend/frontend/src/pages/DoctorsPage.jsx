...
export default function DoctorsPage() {
  ...
  // Available DOCTOR profiles - exclude those already linked to a doctor record (when creating new)
      ...
      {editing === 'new' && availableDoctorProfiles.length === 0 ? (
        <div className="card" style={{ background: '#fff7e6', border: '1px solid #f5c067', padding: '12px 16px', marginBottom: 12 }}>
          ...
          <p className="muted" style={{ margin: '4px 0 0' }}>
            Vui lòng vào trang <strong>Tài khoản</strong> để tạo tài khoản với vai trò "Bác sĩ" trước, rồi quay lại đây để liên kết với hồ sơ.
          </p>
        </div>
      ) : null}
      {editing ? <EntityForm title={editing === 'new' ? 'Tạo hồ sơ bác sĩ' : 'Cập nhật hồ sơ bác sĩ'} fields={fields} value={form} onChange={(name, value) => setForm({ ...form, [name]: value })} onSubmit={submit} onCancel={() => { setEditing(null); setForm(empty); }} submitLabel={editing === 'new' ? 'Tạo' : 'Lưu'} /> : null}
