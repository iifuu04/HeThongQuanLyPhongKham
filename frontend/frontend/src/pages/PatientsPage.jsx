...
const empty = { profileId: '', firstName: '', lastName: '', dateOfBirth: '', gender: 'Male', phone: '', address: '', history: '' };
...
export default function PatientsPage() {
  ...
  const columns = [
    ...
    { key: 'actions', label: 'Thao tác', render: (row) => <div className="action-row"><button className="text-button" onClick={() => setSelectedId(row.id)}>Xem hồ sơ</button><button className="text-button" onClick={() => { const p = db.profiles.find(x => x.id === row.profile_id); setForm({ profileId: row.profile_id, firstName: p?.first_name || '', lastName: p?.last_name || '', dateOfBirth: row.date_of_birth, gender: row.gender || p?.gender || 'Male', phone: row.phone || p?.phone || '', address: row.address || p?.address || '' }); setEditing(row.id); }}>Cập nhật</button></div> },
  ];
  ...
  function submit(event) {
    ...
    if (editing !== 'new') {
      // editing now stores patient.id (e.g., "P00001")
