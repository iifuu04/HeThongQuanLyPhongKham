...
export default function SpecialtiesPage() {
  ...
  const columns = [
    ...
    { key: 'establishAt', label: 'Ngày thành lập', render: (row) => formatDate(row.establish_at || row.establishAt) },
    { key: 'description', label: 'Mô tả' },
    { key: 'status', label: 'Trạng thái', render: (row) => <StatusBadge value={row.status} /> },
    { key: 'actions', label: 'Thao tác', render: (row) => <button className="text-button" onClick={() => { setEditing(row.id); setForm({ name: row.name, establishAt: row.establish_at || row.establishAt, description: row.description, status: row.status }); }}>Cập nhật</button> },
  ];
