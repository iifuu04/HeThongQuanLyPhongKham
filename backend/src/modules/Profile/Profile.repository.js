...
// Schema: Profiles(id INT, username VARCHAR, password_hash VARCHAR, role ENUM, first_name VARCHAR, last_name VARCHAR, date_of_birth DATE, gender ENUM, email VARCHAR, phone VARCHAR, address VARCHAR, created_at TIMESTAMP, updated_at TIMESTAMP, is_deleted BOOLEAN)
...
class Profile {
    ...
    static async generateId(role) {
        ...
        if (rows.length > 0) {
            ...
            // Extract number from format like "P00001" or "D00001"
            const match = lastId.match(/[PD](\d+)/);
            if (match) {
    ...
    /**
     * Get profile by ID (excluding password_hash for security)
     */
    ...
    /**
     * Get profile by username (including password_hash for login verification)
     */
