...
class WorkScheduleService {
    ...
    async createSchedule(data) {
        try {
            ...
            if (existingSchedule) {
                return { success: false, message: 'Bác sĩ đã có lịch làm việc cho ca này vào ngày này' };
            }

            // Additional check: doctor cannot have overlapping shifts on the same date
            const sameDaySchedules = await WorkScheduleRepository.getSchedulesByFilters({
                doctor_id, work_date: normalizedWorkDate
            });
            const overlap = (sameDaySchedules || []).find(s => {
                if (!s.shift_start_time || !s.shift_end_time) return false;
                // Overlap if (newStart < existingEnd) AND (newEnd > existingStart)
                return shift.start_time < s.shift_end_time && shift.end_time > s.shift_start_time;
            });
            if (overlap) {
                return {
                    success: false,
                    message: `Bác sĩ đã có ca làm việc trùng giờ ngày này (${overlap.shift_start_time}–${overlap.shift_end_time})`
                };
            }
