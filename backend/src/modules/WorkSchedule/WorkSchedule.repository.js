...
// Schema: Work_Schedules(id INT, doctor_id VARCHAR(6), clinic_id INT, shift_id INT, work_date DATE)
// Join: Work_Schedules + Doctors + Profiles + Specialties + Clinics + Shifts
...
class WorkScheduleRepository {
    ...
    static async getScheduleById(id) {
        const [rows] = await db.execute(
            `SELECT ws.*,
                    d.profile_id,
                    p.username, p.first_name, p.last_name, p.phone, p.email,
                    s.name AS specialty_name, s.id AS specialty_id,
                    c.name AS clinic_name, c.location AS clinic_location, c.id AS clinic_id,
                    sh.start_time, sh.end_time, sh.max_patients
    ...
    static async getSchedulesByDoctor(doctorId) {
        const [rows] = await db.execute(
            `SELECT ws.*,
             ...
             LEFT JOIN Shifts sh ON ws.shift_id = sh.id
             WHERE ws.doctor_id = ?
             ORDER BY ws.work_date, sh.start_time`,
            [doctorId]
        );
    ...
    static async getSchedulesByFilters(filters) {
        ...
        sql += ` ORDER BY ws.work_date, sh.start_time`;
